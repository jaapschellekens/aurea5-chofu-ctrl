...
void mqtt_ontvang(int len){
  ...
  if(topic == MQTT_PREFIX "/cmd/lcd"){
    ...
    if(payload == "auto" || payload == "handmatig" || payload == "water" ||
      ...
      if(bron == Bron::ADAM && modus != Modus::FF_WATER){
        ...
        mqttClient.beginMessage(MQTT_PREFIX "/bron", true); mqttClient.print("mqtt"); mqttClient.endMessage();
      }
      eeprom_save();
  ...
  else if(topic == MQTT_PREFIX "/cmd/bron"){
    if(payload == "adam"){
      if(!adam_beschikbaar()){
    ...
    mqttClient.beginMessage(MQTT_PREFIX "/bron", true); mqttClient.print(bron_naar_str(bron)); mqttClient.endMessage();
  }
...
void discovery_fase3(){
  ...
  if(adam_beschikbaar()){
    pl = "{\"name\":\"Chofu Bron\",\"uniq_id\":\"" HA_NODE "_bron\",\"cmd_t\":\"" MQTT_PREFIX "/cmd/bron\",\"stat_t\":\"" MQTT_PREFIX "/bron\",\"options\":[\"mqtt\",\"adam\"]," + avty + "," + dev + "}";
    disco_pub("homeassistant/select/" HA_NODE "/bron/config", pl);
    pl = "{\"name\":\"Chofu Adam Status\",\"uniq_id\":\"" HA_NODE "_adam_status\",\"stat_t\":\"" MQTT_PREFIX "/adam/status\"," + avty + "," + dev + "}";
    disco_pub("homeassistant/sensor/" HA_NODE "/adam_status/config", pl);
    pl = "{\"name\":\"Chofu Adam Leider\",\"uniq_id\":\"" HA_NODE "_adam_leider\",\"stat_t\":\"" MQTT_PREFIX "/adam/leider\"," + avty + "," + dev + "}";
    disco_pub("homeassistant/sensor/" HA_NODE "/adam_leider/config", pl);
  }
...
void stuur_data(){
  ...
  mqttClient.beginMessage(MQTT_PREFIX "/bron", true);mqttClient.print(bron_naar_str(bron));mqttClient.endMessage();
  mqttClient.beginMessage(MQTT_PREFIX "/adam/status");mqttClient.print(adam_status_str());mqttClient.endMessage();
  mqttClient.beginMessage(MQTT_PREFIX "/adam/leider");mqttClient.print(adam_leider_naam());mqttClient.endMessage();
